...
export default function OngoingAnime() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => animeApi.getOngoing(page),
  });

  const animeList = useMemo(() => {
    const list = data?.data?.animeList || [];
    return list.filter((a) => matchFilters(a, filters));
  }, [data, filters]);
    ...
    <div>
      <div className="mb-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-heading font-bold">Anime Ongoing</h1>
        </div>
        <p className="text-sm text-muted-foreground mt-1">
          Daftar anime yang sedang tayang saat ini
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <>
          <AnimeGrid animeList={animeList} subtitle={`${animeList.length} anime ditemukan`} />
          <Pagination
